...
function updateTabUi() {
  ...
  el.itemManageBtn.textContent = isAccessory ? "악세사리 관리" : isSpecial ? "보스 컬렉 관리" : "탈것 관리";
  el.itemManageTitle.textContent = isAccessory ? "악세사리 관리" : isSpecial ? "보스 컬렉 관리" : "탈것 관리";
  el.itemNameHeader.textContent = isAccessory ? "악세사리명" : isSpecial ? "보스 컬렉명" : "탈것명";
  el.itemMaxHeader.classList.toggle("hidden", !isAccessory);
  el.addItemBtn.textContent = isAccessory ? "악세사리 추가" : isSpecial ? "보스 컬렉 추가" : "탈것 추가";
}
...
function syncDraftState() {
  ...
  const items = state.activeTab === "special" ? state.bossItems : state.mountItems;
  const records = state.activeTab === "special" ? state.memberBossCollections : state.memberMounts;

  items.forEach((item) => {
    const record = records.find(
      (entry) => entry.member_id === editableMember.id && String(state.activeTab === "special" ? entry.boss_collection_id : entry.mount_id) === String(item.id)
    );
...
function ensureDraftRow(memberId) {
  ...
  if (!state.draftAllRows[memberId]) {
    ...
    if (state.activeTab === "accessory") {
      ...
      const records = state.activeTab === "special" ? state.memberBossCollections : state.memberMounts;
      items.forEach((item) => {
        const record = records.find((entry) => entry.member_id === memberId && String(state.activeTab === "special" ? entry.boss_collection_id : entry.mount_id) === String(item.id));
...
function renderMountSummaryTable() {
  ...
  const headers = [
    ...
    ...items.map((item) => `<th class="item-col-header">${escapeHtml(item.name)}</th>`),
  ...
  if (filteredMembers.length === 0) {
    el.summaryTableBody.innerHTML = `
      <tr>
        <td class="empty-row" colspan="${items.length + 5}">표시할 길드원이 없습니다.</td>
      </tr>
    `;
    return;
  }
...
function renderItemManageTable() {
  const items = state.activeTab === "accessory" ? state.accessoryGroups : state.activeTab === "special" ? state.bossItems : state.mountItems;
...
async function saveMountEditableRow(memberId) {
  ...
  const upsertPayload = items.map((item) => ({
    member_id: memberId,
    [state.activeTab === "special" ? "boss_collection_id" : "mount_id"]: item.id,
    owned: Boolean(state.overallEditMode ? draftRow?.ownedMap?.[item.id] : state.draftOwnedMap[item.id])
  }));

  const upsertRes = await supabase
    .from(state.activeTab === "special" ? "member_boss_collections" : "member_mounts")
    .upsert(upsertPayload, { onConflict: state.activeTab === "special" ? "member_id,boss_collection_id" : "member_id,mount_id" });
...
function getOwnedValue(memberId, itemId) {
  const records = state.activeTab === "special" ? state.memberBossCollections : state.memberMounts;
  const record = records.find(
    (entry) => entry.member_id === memberId && String(state.activeTab === "special" ? entry.boss_collection_id : entry.mount_id) === String(itemId)
  );
