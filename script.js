...
/* ===== New Distribution Rebuild ===== */
const NEW_DIST_DEFAULT_DEDUCTIONS = [
  { id: crypto.randomUUID(), name: "길드운영비", type: "percent", value: 10 },
  { id: crypto.randomUUID(), name: "길드장", type: "percent", value: 5 },
  { id: crypto.randomUUID(), name: "총무비", type: "percent", value: 3 }
];

function cloneDefaultDeductions() {
  return NEW_DIST_DEFAULT_DEDUCTIONS.map((item) => ({
...
function createEmptyDistributionState() {
  return {
    ...
    activeLogEdit: null,
    mainland: {
      deductions: cloneDefaultDeductions(),
      logs: [],
      results: []
    },
    world: {
...
function getNewDistRoot() {
  return document.getElementById("distributionContent");
}

function getNewDistEl(id) {
  return document.getElementById(id);
}

function calculateNewDistAllocation(total, ratio) {
  return Math.floor((Number(total || 0) * Number(ratio || 0)) / 100);
}

function calculateNewDistDeductionAmount(baseAmount, item) {
  const value = Math.max(0, Number(item?.value || 0));
  if (item?.type === "amount") return Math.floor(value);
  return Math.floor(baseAmount * (value / 100));
}

function summarizeNewDistGroup(groupKey) {
  const distribution = state.distribution;
  const totalDiamond = Math.max(0, Number(distribution.totalDiamond || 0));
  const ratio = groupKey === "mainland" ? Number(distribution.mainlandRatio || 0) : Number(distribution.worldRatio || 0);
  const allocated = calculateNewDistAllocation(totalDiamond, ratio);
  const group = distribution[groupKey];
  const deductionTotal = group.deductions.reduce((sum, item) => sum + calculateNewDistDeductionAmount(allocated, item), 0);
  const actual = Math.max(0, allocated - deductionTotal);
  const activeResults = (group.results || []).filter((row) => !(row.note || "").includes("탈퇴"));
  const totalPoints = activeResults.reduce((sum, row) => sum + Number(row.points || 0), 0);
  const perPoint = totalPoints > 0 ? actual / totalPoints : 0;
  const paid = activeResults.reduce((sum, row) => sum + Math.floor((Number(row.points || 0) * perPoint)), 0);
  const remain = Math.max(0, actual - paid);
  return { allocated, deductionTotal, actual, totalPoints, perPoint, remain };
}

function formatNewDistNumber(value) {
  return Number(value || 0).toLocaleString("ko-KR");
}

function formatNewDistDecimal(value) {
  return Number(value || 0).toLocaleString("ko-KR", { minimumFractionDigits: 1, maximumFractionDigits: 1 });
}

function renderNewDistCommonSummary() {
  const distribution = state.distribution;
  const total = Math.max(0, Number(distribution.totalDiamond || 0));
  getNewDistEl("newDistSummaryTotalDiamond").textContent = formatNewDistNumber(total);
  getNewDistEl("newDistSummaryMainlandAmount").textContent = formatNewDistNumber(calculateNewDistAllocation(total, distribution.mainlandRatio));
  getNewDistEl("newDistSummaryWorldAmount").textContent = formatNewDistNumber(calculateNewDistAllocation(total, distribution.worldRatio));
  getNewDistEl("newDistSummaryLogStatus").textContent = distribution.logStatus || "대기중";
}

function renderNewDistDeductions(groupKey) {
  const distribution = state.distribution;
  const group = distribution[groupKey];
  const tbody = getNewDistEl(groupKey === "mainland" ? "newDistMainlandDeductionBody" : "newDistWorldDeductionBody");
  const summary = summarizeNewDistGroup(groupKey);

  tbody.innerHTML = group.deductions.map((item) => `
    <tr>
      <td><input class="input-sm" type="text" data-role="new-dist-deduction-name" data-group="${groupKey}" data-id="${item.id}" value="${escapeAttr(item.name)}"></td>
      <td>
        <select class="select-sm" data-role="new-dist-deduction-type" data-group="${groupKey}" data-id="${item.id}">
          <option value="percent" ${item.type === "percent" ? "selected" : ""}>%</option>
          <option value="amount" ${item.type === "amount" ? "selected" : ""}>직접금액</option>
        </select>
      </td>
      <td><input class="input-sm" type="number" min="0" step="0.01" data-role="new-dist-deduction-value" data-group="${groupKey}" data-id="${item.id}" value="${escapeAttr(item.value)}"></td>
      <td class="is-right">${formatNewDistNumber(calculateNewDistDeductionAmount(summary.allocated, item))}</td>
      <td class="is-center"><button class="btn btn-danger btn-sm" type="button" data-role="new-dist-deduction-delete" data-group="${groupKey}" data-id="${item.id}">삭제</button></td>
    </tr>
  `).join("");

  if (!group.deductions.length) {
    tbody.innerHTML = `<tr><td class="distribution-empty-row" colspan="5">등록된 공제 항목이 없습니다.</td></tr>`;
  }

  const prefix = groupKey === "mainland" ? "newDistMainland" : "newDistWorld";
  getNewDistEl(prefix + "ChipDeduction").textContent = formatNewDistNumber(summary.deductionTotal);
  getNewDistEl(prefix + "ChipActual").textContent = formatNewDistNumber(summary.actual);
  getNewDistEl(prefix + "Allocated").textContent = formatNewDistNumber(summary.allocated);
  getNewDistEl(prefix + "DeductionTotal").textContent = formatNewDistNumber(summary.deductionTotal);
  getNewDistEl(prefix + "Actual").textContent = formatNewDistNumber(summary.actual);
  getNewDistEl(prefix + "Points").textContent = formatNewDistNumber(summary.totalPoints);
  getNewDistEl(prefix + "PerPoint").textContent = summary.totalPoints > 0 ? formatNewDistDecimal(summary.perPoint) : "0";
  getNewDistEl(prefix + "Remain").textContent = formatNewDistNumber(summary.remain);
}

function renderNewDistLogs(groupKey) {
  const group = state.distribution[groupKey];
  const tbody = getNewDistEl(groupKey === "mainland" ? "newDistMainlandLogBody" : "newDistWorldLogBody");
  if (!group.logs.length) {
    tbody.innerHTML = `<tr><td class="distribution-empty-row" colspan="9">작업 로그가 없습니다.</td></tr>`;
    return;
  }
  tbody.innerHTML = group.logs.map((row, index) => `
    <tr>
      <td class="is-center">${index + 1}</td>
      <td>${escapeHtml(row.date || "-")}</td>
      <td>${escapeHtml(row.time || "-")}</td>
      <td>${escapeHtml(row.boss || "-")}</td>
      <td class="is-center"><span class="badge ${groupKey === "mainland" ? "badge-mainland" : "badge-world"}">${groupKey === "mainland" ? "본토" : "월드"}</span></td>
      <td>${escapeHtml(row.cutter || "-")}</td>
      <td>${escapeHtml((row.excelParticipants || []).join(", "))}</td>
      <td>${escapeHtml((row.workParticipants || []).join(", "))}</td>
      <td class="is-center"><button class="btn btn-outline btn-sm" type="button" data-role="new-dist-log-edit" data-group="${groupKey}" data-index="${index}">수정</button></td>
    </tr>
  `).join("");
}

function renderNewDistResults(groupKey) {
  const group = state.distribution[groupKey];
  const tbody = getNewDistEl(groupKey === "mainland" ? "newDistMainlandResultBody" : "newDistWorldResultBody");
  const summary = summarizeNewDistGroup(groupKey);
  if (!group.results.length) {
    tbody.innerHTML = `<tr><td class="distribution-empty-row" colspan="7">계산 결과가 없습니다.</td></tr>`;
    return;
  }
  tbody.innerHTML = group.results.map((row, index) => {
    const ratio = summary.totalPoints > 0 && !(row.note || "").includes("탈퇴") ? Number(row.points || 0) / summary.totalPoints : 0;
    const rawDiamond = !(row.note || "").includes("탈퇴") ? Number(row.points || 0) * summary.perPoint : 0;
    const finalDiamond = Math.floor(rawDiamond);
    return `
      <tr class="${(row.note || "").includes("탈퇴") ? "distribution-retired-row" : ""}">
        <td class="is-center">${index + 1}</td>
        <td>${escapeHtml(row.memberName || "-")}</td>
        <td class="is-right">${formatNewDistNumber(row.points || 0)}</td>
        <td class="is-right">${(ratio * 100).toFixed(2)}%</td>
        <td class="is-right">${formatNewDistDecimal(rawDiamond)}</td>
        <td class="is-right">${formatNewDistNumber(finalDiamond)}</td>
        <td>${escapeHtml(row.note || "-")}</td>
      </tr>
    `;
  }).join("");
...
function renderNewDistBossManageTable() {
  const tbody = getNewDistEl("newDistBossManageTableBody");
  const rows = state.distribution.bossMasters || [];
  if (!rows.length) {
    tbody.innerHTML = `<tr><td colspan="5">등록된 보스가 없습니다.</td></tr>`;
    return;
  }
  tbody.innerHTML = rows.map((row, index) => `
    <tr>
      <td>${index + 1}</td>
      <td><input class="inline-power-input" style="width:100%;" type="text" data-role="new-dist-boss-name" data-id="${row.id}" value="${escapeAttr(row.name)}"></td>
      <td><input class="inline-power-input" type="number" min="1" step="1" data-role="new-dist-boss-point" data-id="${row.id}" value="${escapeAttr(row.point)}"></td>
      <td>
        <select class="select-sm" data-role="new-dist-boss-type" data-id="${row.id}">
          <option value="mainland" ${row.type === "mainland" ? "selected" : ""}>본토</option>
          <option value="world" ${row.type === "world" ? "selected" : ""}>월드</option>
        </select>
      </td>
      <td><button class="text-btn danger" type="button" data-role="new-dist-boss-delete" data-id="${row.id}">삭제</button></td>
    </tr>
  `).join("");
}

function renderNewDistNameRuleTable() {
  const tbody = getNewDistEl("newDistNameRuleTableBody");
  const rows = state.distribution.nameRules || [];
  if (!rows.length) {
    tbody.innerHTML = `<tr><td colspan="4">등록된 규칙이 없습니다.</td></tr>`;
    return;
  }
  tbody.innerHTML = rows.map((row, index) => `
    <tr>
      <td>${index + 1}</td>
      <td><input class="inline-power-input" style="width:100%;" type="text" data-role="new-dist-rule-source" data-id="${row.id}" value="${escapeAttr(row.source)}"></td>
      <td><input class="inline-power-input" style="width:100%;" type="text" data-role="new-dist-rule-target" data-id="${row.id}" value="${escapeAttr(row.target)}"></td>
      <td><button class="text-btn danger" type="button" data-role="new-dist-rule-delete" data-id="${row.id}">삭제</button></td>
    </tr>
  `).join("");
}
...
function renderDistributionTab() {
  const distribution = state.distribution;
  getNewDistEl("newDistTotalDiamond").value = distribution.totalDiamond || "";
  getNewDistEl("newDistMainlandRatio").value = distribution.mainlandRatio ?? "";
  getNewDistEl("newDistWorldRatio").value = distribution.worldRatio ?? "";
  const panels = {
    mainland: getNewDistEl("newDistPanelMainland"),
    world: getNewDistEl("newDistPanelWorld")
  };
  document.querySelectorAll(".new-dist-subtab").forEach((btn) => {
    btn.classList.toggle("active", btn.dataset.subtab === distribution.subtab);
  });
  panels.mainland.classList.toggle("active", distribution.subtab === "mainland");
  panels.world.classList.toggle("active", distribution.subtab === "world");

  renderNewDistCommonSummary();
  renderNewDistDeductions("mainland");
  renderNewDistDeductions("world");
  renderNewDistLogs("mainland");
  renderNewDistLogs("world");
  renderNewDistResults("mainland");
  renderNewDistResults("world");
  renderNewDistBossManageTable();
  renderNewDistNameRuleTable();
}

function renderHistoryTab() {
  const content = getNewDistEl("historyContent");
...
function updateTabUi() {
  ...
  el.historyHeaderActions.classList.add("hidden");

  if (isDistribution) {
    el.mainCardTitle.textContent = "분배";
    el.tableGuideText.textContent = "새 분배 구조 기준으로 본토/월드 분배를 관리합니다.";
  } else if (isHistory) {
    el.mainCardTitle.textContent = "분배 이력";
    el.tableGuideText.textContent = "새 분배 구조 기준으로 분배 이력은 다음 단계에서 다시 연결합니다.";
  } else {
    el.mainCardTitle.textContent = "전체 현황";
  }
  ...
  el.importOpenBtn.classList.toggle("hidden", !(state.activeTab === "mount" || state.activeTab === "boss" || state.activeTab === "accessory"));
  el.importOpenBtn.disabled = isSpecial || isDistribution || isHistory || isPower || state.isBulkSaving;
  el.itemManageBtn.textContent = state.activeTab === "accessory" ? "악세사리 관리" : state.activeTab === "boss" ? "보스컬렉 관리" : "탈것 관리";
  el.itemManageTitle.textContent = state.activeTab === "accessory" ? "악세사리 관리" : state.activeTab === "boss" ? "보스컬렉 관리" : "탈것 관리";
  el.itemNameHeader.textContent = state.activeTab === "accessory" ? "악세사리명" : state.activeTab === "boss" ? "보스컬렉명" : "탈것명";
  el.itemMaxHeader.classList.toggle("hidden", state.activeTab !== "accessory");
  el.addItemBtn.textContent = state.activeTab === "accessory" ? "악세사리 추가" : state.activeTab === "boss" ? "보스컬렉 추가" : "탈것 추가";
  el.bulkEditBtn.classList.toggle("hidden", state.overallEditMode);
  el.bulkSaveBtn.classList.toggle("hidden", !state.overallEditMode);
  el.bulkCancelBtn.classList.toggle("hidden", !state.overallEditMode);
}
...
function syncNewDistCommonInputs() {
  state.distribution.totalDiamond = Math.max(0, Number(getNewDistEl("newDistTotalDiamond").value || 0));
  state.distribution.mainlandRatio = Math.max(0, Number(getNewDistEl("newDistMainlandRatio").value || 0));
  state.distribution.worldRatio = Math.max(0, Number(getNewDistEl("newDistWorldRatio").value || 0));
}

function bindNewDistributionUi() {
  const root = getNewDistRoot();
  if (!root) return;

  getNewDistEl("newDistResetBtn").addEventListener("click", () => {
    state.distribution = createEmptyDistributionState();
    renderDistributionTab();
  });

  getNewDistEl("newDistLoadBtn").addEventListener("click", () => {
    syncNewDistCommonInputs();
    const file = getNewDistEl("newDistFileInput").files?.[0];
    if (!file) {
      alert("보스로그 엑셀 파일을 선택해주세요.");
      return;
    }
    state.distribution.workbookName = file.name;
    state.distribution.logStatus = `로드 완료 (${file.name})`;
    renderDistributionTab();
  });

  ["newDistTotalDiamond","newDistMainlandRatio","newDistWorldRatio"].forEach((id) => {
    getNewDistEl(id).addEventListener("input", () => {
      syncNewDistCommonInputs();
      renderDistributionTab();
    });
  });
  ...
  document.querySelectorAll(".new-dist-subtab").forEach((btn) => {
    btn.addEventListener("click", () => {
      state.distribution.subtab = btn.dataset.subtab;
      renderDistributionTab();
    });
  });

  getNewDistEl("newDistAddMainlandDeductionBtn").addEventListener("click", () => {
    state.distribution.mainland.deductions.push({ id: crypto.randomUUID(), name: "", type: "percent", value: 0 });
    renderDistributionTab();
  });
  getNewDistEl("newDistAddWorldDeductionBtn").addEventListener("click", () => {
    state.distribution.world.deductions.push({ id: crypto.randomUUID(), name: "", type: "percent", value: 0 });
    renderDistributionTab();
  });
  ...
  root.addEventListener("input", (event) => {
    ...
    if (role.startsWith("new-dist-deduction-")) {
      const item = state.distribution[group].deductions.find((entry) => entry.id === itemId);
      if (!item) return;
      if (role === "new-dist-deduction-name") item.name = target.value;
      if (role === "new-dist-deduction-type") item.type = target.value;
      if (role === "new-dist-deduction-value") item.value = Math.max(0, Number(target.value || 0));
      renderDistributionTab();
    }
  });
  ...
  root.addEventListener("click", (event) => {
    ...
    if (!button) return;
    const { role, group, id, index } = button.dataset;
    if (role === "new-dist-deduction-delete") {
      state.distribution[group].deductions = state.distribution[group].deductions.filter((entry) => entry.id !== id);
      renderDistributionTab();
      return;
    }
    if (role === "new-dist-log-edit") {
      const row = state.distribution[group].logs[Number(index)];
      state.distribution.activeLogEdit = { group, index: Number(index) };
      getNewDistEl("newDistLogEditCurrent").value = (row.workParticipants || []).join(", ");
      getNewDistEl("newDistLogEditEdited").value = (row.workParticipants || []).join(", ");
      openModal(getNewDistEl("newDistLogEditModalBackdrop"));
      return;
    }
  });

  getNewDistEl("newDistBossManageOpenBtn").addEventListener("click", () => openModal(getNewDistEl("newDistBossManageModalBackdrop")));
  getNewDistEl("newDistNameRuleOpenBtn").addEventListener("click", () => openModal(getNewDistEl("newDistNameRuleModalBackdrop")));
  getNewDistEl("newDistBossManageCloseBtn").addEventListener("click", () => closeModal(getNewDistEl("newDistBossManageModalBackdrop")));
  getNewDistEl("newDistNameRuleCloseBtn").addEventListener("click", () => closeModal(getNewDistEl("newDistNameRuleModalBackdrop")));
  getNewDistEl("newDistLogEditCloseBtn").addEventListener("click", () => closeModal(getNewDistEl("newDistLogEditModalBackdrop")));
  getNewDistEl("newDistLogEditCancelBtn").addEventListener("click", () => closeModal(getNewDistEl("newDistLogEditModalBackdrop")));

  getNewDistEl("newDistBossAddBtn").addEventListener("click", () => {
    state.distribution.bossMasters.push({ id: crypto.randomUUID(), name: "", point: 1, type: "mainland" });
    renderNewDistBossManageTable();
  });
  getNewDistEl("newDistNameRuleAddBtn").addEventListener("click", () => {
    state.distribution.nameRules.push({ id: crypto.randomUUID(), source: "", target: "" });
    renderNewDistNameRuleTable();
  });

  getNewDistEl("newDistBossManageTableBody").addEventListener("input", (event) => {
    const target = event.target;
    const item = state.distribution.bossMasters.find((entry) => entry.id === target.dataset.id);
    if (!item) return;
    if (target.dataset.role === "new-dist-boss-name") item.name = target.value;
    if (target.dataset.role === "new-dist-boss-point") item.point = Math.max(1, Math.floor(Number(target.value || 1)));
    if (target.dataset.role === "new-dist-boss-type") item.type = target.value;
  });
  getNewDistEl("newDistBossManageTableBody").addEventListener("click", (event) => {
    const button = event.target.closest("button[data-role='new-dist-boss-delete']");
    if (!button) return;
    state.distribution.bossMasters = state.distribution.bossMasters.filter((entry) => entry.id !== button.dataset.id);
    renderNewDistBossManageTable();
  });
  getNewDistEl("newDistNameRuleTableBody").addEventListener("input", (event) => {
    const target = event.target;
    const item = state.distribution.nameRules.find((entry) => entry.id === target.dataset.id);
    if (!item) return;
    if (target.dataset.role === "new-dist-rule-source") item.source = target.value;
    if (target.dataset.role === "new-dist-rule-target") item.target = target.value;
  });
  getNewDistEl("newDistNameRuleTableBody").addEventListener("click", (event) => {
    const button = event.target.closest("button[data-role='new-dist-rule-delete']");
    if (!button) return;
    state.distribution.nameRules = state.distribution.nameRules.filter((entry) => entry.id !== button.dataset.id);
    renderNewDistNameRuleTable();
  });

  getNewDistEl("newDistLogEditApplyBtn").addEventListener("click", () => {
    const active = state.distribution.activeLogEdit;
    if (!active) return;
    const text = getNewDistEl("newDistLogEditEdited").value;
    state.distribution[active.group].logs[active.index].workParticipants = text.split(",").map((v) => v.trim()).filter(Boolean);
    closeModal(getNewDistEl("newDistLogEditModalBackdrop"));
    renderDistributionTab();
  });

  getNewDistEl("newDistMainlandCalculateBtn").addEventListener("click", () => {
    syncNewDistCommonInputs();
    renderDistributionTab();
    alert("본토 계산 UI 기준으로 갱신했습니다.");
  });
  getNewDistEl("newDistWorldCalculateBtn").addEventListener("click", () => {
    syncNewDistCommonInputs();
    renderDistributionTab();
    alert("월드 계산 UI 기준으로 갱신했습니다.");
  });

  [getNewDistEl("newDistBossManageModalBackdrop"), getNewDistEl("newDistNameRuleModalBackdrop"), getNewDistEl("newDistLogEditModalBackdrop")].forEach((backdrop) => {
    backdrop.addEventListener("click", (event) => {
